...
// Global codon_table
static CODON_TABLE: OnceLock<HashMap<u8, Vec<&str>>> = OnceLock::new();
...
fn init_codon_table() {
    ...
    CODON_TABLE.set(codon_table).expect("Somehing went wrong. Conflict may happend.");
}
...
fn encrypt_data(chunks: Vec<String>) -> Vec<String> {
    let mut encrypted_data: Vec<String> = Vec::new();
    for chunk in chunks {
        if let Ok(value) = u8::from_str_radix(&chunk, 2) {
            if let Some(codon_choices) = CODON_TABLE.get().unwrap().get(&value) {
...
fn main() {
    ...
    let encrypted_data = encrypt_data(binary_chunks);
